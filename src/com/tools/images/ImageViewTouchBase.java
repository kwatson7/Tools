...
implements IDisposable {
	...
	private boolean didWeUpdate = true;
	protected void updateRect( RectF bitmapRect, RectF scrollRect )
	{
		// output, did we move anything?
		boolean update = true;
		...
		if ( bitmapRect.top >= 0 && bitmapRect.bottom <= height ) {scrollRect.top = 0; update = true;}
		if ( bitmapRect.left >= 0 && bitmapRect.right <= width ) {scrollRect.left = 0; update = true;}
		if ( bitmapRect.top + scrollRect.top >= 0 && bitmapRect.bottom > height ) {scrollRect.top = (int)( 0 - bitmapRect.top ); update = true;}
		if ( bitmapRect.bottom + scrollRect.top <= ( height - 0 ) && bitmapRect.top < 0 ) {scrollRect.top = (int)( ( height - 0 ) - bitmapRect.bottom ); update = true;}
		if ( bitmapRect.left + scrollRect.left >= 0 ) {scrollRect.left = (int)( 0 - bitmapRect.left ); update = false;}
		if ( bitmapRect.right + scrollRect.left <= ( width - 0 ) ) {scrollRect.left = (int)( ( width - 0 ) - bitmapRect.right ); update = false;}

		didWeUpdate = update;
	}

	public boolean didWeUpdate(){
		return didWeUpdate;
	}
